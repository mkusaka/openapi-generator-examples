...
> User CreateUsersWithListInput(ctx).User(user).Execute()
...
func main() {
    user := []openapiclient.User{*openapiclient.NewUser()} // []User |  (optional)
    ...
    // response from `CreateUsersWithListInput`: User
    fmt.Fprintf(os.Stdout, "Response from `UserApi.CreateUsersWithListInput`: %v\n", resp)
}
```
...
func main() {
    ...
    user := *openapiclient.NewUser() // User | Update an existent user in the store (optional)
